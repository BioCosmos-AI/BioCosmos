...
# No longer required, we can use the df as is
...
class TreeOfLifeWebDataset:
    ...
    def __init__(
        self, webdataset_path, df, label_column, transform=None, world_size=1, rank=0, batch_size=64
    ):
        # ASSUMPTIONS ON HEADER LABELS FOR DF: 
        # uuid | shard_id | img_only_cluster_id | img_txt_avg_cluster_id | img_txt_concat_cluster_id
        # id column names could be different (coordinate with Thomas) - to be selected via label column
        ...
        # Label column depends on arg being passed from SLURM, one of : img_only_cluster_id, img_txt_avg_cluster_id, img_txt_concat_cluster_id
        # select based on used clustering method
        self.label_column = label_column

        # Create mapping from uuid to label
        self.uuid_to_label = dict(zip(df["uuid"], df[self.label_column]))
        ...
        # Instead of using all shard ids, only use the ones that are relevant to rank_uuids
        self.rank_shard_ids = sorted(
            df.loc[df["uuid"].isin(rank_uuid_set), "shard_id"].unique()
        )
    ...
    def create_loader(self, epoch=0):
        ...
        # create shard_urls ONLY for the current rank
        rank_urls = [
            f"{self.webdataset_path}/shard-{shard_id}.tar"
            for shard_id in self.rank_shard_ids
        ]
        ...
        def select_by_rank(sample):
            ...
            uuid = sample["__key__"].rsplit(".")[0]
            # Keep only if this UUID is assigned to this rank
            return uuid in self.rank_uuid_set


        # Note: we are not using DataLoaders, so no need for workers (workers=0)

        # Create the pipeline
        ds = wds.DataPipeline(
            wds.SimpleShardList(rank_urls),
            ...
            wds.map(
                lambda sample: (
                    ...
                    self.uuid_to_label.get(sample["__key__"].rsplit(".")[0], 0),
                )
            ),
...
def main():
    ...
    # Data parameters
    parser.add_argument(
        "--cluster-label-csv", required=True, help="Path to CSV with uuid-to-cluster label mapping"
    )
    parser.add_argument(
        "--cluster-label-column", required=True, help="Column to be used as pseudo-class (cluster_id) for training"
    )
    ...
    df = pd.read_csv(args.cluster_label_csv)
    ...
    if global_rank == 0:
        logger.info("Loading pretrained ViT-B/16 model (openai)")

    model, _, preprocess_train = open_clip.create_model_and_transforms(
        "ViT-B/16" #, pretrained="laion2b_s32b_b82k" # BioClip2 basis model
    )
    ...
    module_partial_fc = PartialFC_V2(
        ...
        num_classes=len(df[args.cluster_label_column].unique()),  # Always use checkpoint's class count
