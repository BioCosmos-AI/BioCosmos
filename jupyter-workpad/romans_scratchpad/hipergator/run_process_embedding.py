...
def load_trained_model(state_dict_path):
    ...
    # Step 2: Extract and move the vision tower
    model = model.visual
    model.to(device)
...
def process_shards(split, model, preprocess, output_format, batch_size=128):
    ...
    for shard in shard_files:
        logger.info(f"Now working with {shard}")
        shard_no = shard.split('.tar')[0].split('-')[-1]
        logger.info(f'shard_no is : {shard_no}')
        output_file = os.path.join(shards_dir, f"embeddings/trained_model/train_embeddings_{split}_shard_{shard_no}.{output_format}")
...
def main():
    ...
    if len(sys.argv) != 3:
        logger.error("Usage: python process_embeddings.py <split> <path_to_state_dict>")
        sys.exit(1)
    ...
    model, preprocess = load_trained_model(state_dict_path)
