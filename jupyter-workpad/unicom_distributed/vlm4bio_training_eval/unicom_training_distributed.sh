#!/bin/bash
#SBATCH --job-name=unicom_train_vlm4bio
#SBATCH --nodes=1                   # Request 1 node
#SBATCH --ntasks-per-node=2         # Use 8 GPUs on the node (adjust as needed)
#SBATCH --cpus-per-task=8           # 8 CPUs per GPU for data loading
#SBATCH --partition=hpg-b200
#SBATCH --gres=gpu:b200:2           # Request 2 GPUs
#SBATCH --mem=200gb                 # Memory for the node (high for SQLite DB in memory)
#SBATCH --time=16:00:00           # Maximum runtime 
#SBATCH --output=/home/rdombrovski3.gatech/rdombrovski_scripts/training/logs/unicom_training_distributed_%j.log
...
CHECKPOINT_DIR="/blue/arthur.porto-biocosmos/rdombrovski3.gatech/unicom/"
# LOG_DIR="/home/tdeatherage3.gatech/logs/unicom_train_${SLURM_JOB_ID}"
LOG_DIR="/home/rdombrovski3.gatech/rdombrovski_scripts/training/logs/unicom_training_distributed_${SLURM_JOB_ID}"
EMBEDDING_PATH="/blue/arthur.porto-biocosmos/data/datasets/VLM4Bio/cluster_embeddings.mat"
...
torchrun --nnodes=1 --node_rank=0 --nproc_per_node=$SLURM_NTASKS_PER_NODE --master_addr=$MASTER_ADDR --master_port=$MASTER_PORT \
    /home/rdombrovski3.gatech/rdombrovski_scripts/training/unicom_training_distributed.py \
    --mat-path "$EMBEDDING_PATH" \
