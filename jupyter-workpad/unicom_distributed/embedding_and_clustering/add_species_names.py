...
def setup_logging(log_dir):
    ...
    log_file = os.path.join(log_dir, f"species_name_import_{timestamp}.log")
    ...
    try:
        with tarfile.open(tar_path, "r") as tar:
            ...
            for member in tqdm(
                ...
                if member.name.endswith(".scientific_name.txt"):
                    try:
...
def main():
    parser = argparse.ArgumentParser(
        description="Batch import of species names from tar files to SQLite database"
    )
    ...
    logger.info("Starting batch import of species names")
    logger.info(
