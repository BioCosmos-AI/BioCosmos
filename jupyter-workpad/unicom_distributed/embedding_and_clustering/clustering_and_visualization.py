...
def setup_logging(log_dir):
    ...
    log_file = os.path.join(log_dir, f"clustering_and_viz_{timestamp}.log")
...
def generate_tsne(embeddings, use_gpu, perplexity, logger, n_iter=3000):
    """Generate t-SNE representation using GPU if available."""
    logger.info(
        f"Generating 2D t-SNE projection for {len(embeddings)} samples with {n_iter} iterations"
    )

    # Adjust perplexity if needed (must be smaller than n_samples - 1)
    # adjusted_perplexity = min(perplexity, len(embeddings) - 1)
    adjusted_perplexity = int(np.ceil(np.sqrt(len(embeddings))))
    if adjusted_perplexity < perplexity:
        logger.info(
            f"Adjusted perplexity from {perplexity} to {adjusted_perplexity} due to small dataset size"
        )
    ...
    if use_gpu and CUML_AVAILABLE and torch.cuda.is_available():
        try:
            logger.info(
                f"Attempting GPU-accelerated t-SNE with perplexity={adjusted_perplexity}, iterations={n_iter}"
            )
            tsne = cuTSNE(
                n_components=2,
                random_state=42,
            ...
            if tsne_result is not None:
                ...
                tsne_result = (tsne_result - tsne_min) / (tsne_max - tsne_min + 1e-8)
    ...
    # Fall back to CPU t-SNE
    try:
        logger.info(
            f"Using CPU t-SNE with perplexity={adjusted_perplexity}, iterations={n_iter}"
        )
        tsne = TSNE(
            n_components=2,
            random_state=42,
        ...
        logger.info("Successfully used CPU t-SNE")
        return tsne_result
    except Exception as e:
...
def perform_tsne_and_clustering(
    species_df, embeddings_dict, min_k, max_k, perplexity, use_gpu, logger, n_iter=3000
):
    """Perform t-SNE and clustering, keeping t-SNE coordinates for visualization."""
    ...
    tsne_result = generate_tsne(embeddings, use_gpu, perplexity, logger, n_iter=n_iter)
    ...
    valid_df["x"] = tsne_result[:, 0]
    valid_df["y"] = tsne_result[:, 1]

    # Find optimal number of clusters
    logger.info(f"Finding optimal k for {species_name}")
    optimal_k, score = find_optimal_k(tsne_result, [min_k, max_k], use_gpu, logger)
...
def generate_visualization(
    ...
    output_path = os.path.join(output_dir, f"{safe_name}_tsne_with_images.png")
...
def main():
    parser = argparse.ArgumentParser(
        description="Combined clustering and visualization for specific species"
    )
    parser.add_argument(
        "--db-path",
        type=str,
    ...
    logger.info("Starting combined clustering and visualization script")
    ...
    for species_name in args.species:
        ...
        processed_df, stats, embeddings = perform_tsne_and_clustering(
            species_df,
            embeddings_dict,
            args.min_k,
            args.max_k,
        ...
        tsne_result = np.column_stack((processed_df["x"], processed_df["y"]))
        representatives = get_cluster_representatives(
            processed_df, embeddings, tsne_result, logger
        )
