...
def main(args):
    ...
    out_file_name = os.path.join(run_dir, f"referring_florence2_{args.trait_option}_num_{args.num_queries}.jsonl")
    ...
    with open(out_file_name, 'w') as writer:
        for idx in tqdm(range(len(train_df))):
            ...
            florence_output = run_florence2(model, processor, image, task_prompt, text_input=florence2_bbox)
            ...
            if args.visual_output:
                output_image_path = os.path.join(images_dir, f"{os.path.splitext(img_filename)[0]}_annotated.jpg")
                save_annotated_image(image.copy(), bbox, args.trait_option, generated_text, output_image_path)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--task_option", "-t", type=str, default='referring', choices=['referring'])
    ...
    parser.add_argument("--result_dir", "-o", type=str, default='results/referring', help="path to output")
    ...
    parser.add_argument("--dataset_csv", type=str, required=True, help="path to dataset CSV file")
    parser.add_argument("--visual_output", action="store_true", help="Enable visual output of annotated images")
