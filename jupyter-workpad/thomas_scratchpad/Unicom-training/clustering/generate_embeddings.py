...
class VLM4BioDataset(Dataset):
    def __init__(self, csv_path, base_path, preprocess):
...
def process_and_save_embeddings(csv_path, base_path, batch_size=32, save_every=1000):
    device = "cuda" if torch.cuda.is_available() else "cpu"
    model, preprocess = clip.load("ViT-B/32", device=device)
    model.eval()
    
    dataset = VLM4BioDataset(csv_path, base_path, preprocess)
...
def save_batch_to_mat(image_embeddings, text_embeddings, metadata, file_count):
    save_dict = {
    ...
    savemat(f'embeddings_batch_{file_count}.mat', save_dict)
