    ...
    for category in categories:
        ...
        for chunk_dir in chunk_dirs:
            ...
            for image_file in image_files:
                if os.path.isfile(image_file):
                    ...
                    while os.path.exists(dest_path):
                        dest_path = os.path.join(new_images_dir, f"{base}_{counter}{ext}")
                        counter += 1
            ...
            if os.path.exists(metadata_dir):
                metadata_files = glob.glob(os.path.join(metadata_dir, "*.csv"))
                
                for metadata_file in metadata_files:
                    filename = os.path.basename(metadata_file)
                    dest_path = os.path.join(new_metadata_dir, filename)
                    
                    # If metadata file already exists, merge them
                    if os.path.exists(dest_path):
                        existing_df = pd.read_csv(dest_path)
                        new_df = pd.read_csv(metadata_file)
        ...
        # Uncomment the following lines to remove old structure
        # for chunk_dir in chunk_dirs:
