...
def load_metadata(category_dir):
    ...
    try:
        ...
        return {row['fileNameAsDelivered']: row['scientificName'] 
                for _, row in df.iterrows() if pd.notna(row['fileNameAsDelivered'])}
    except Exception as e:
        logging.error(f"Error loading metadata from {metadata_file}: {str(e)}")
        return {}

def process_vlm4bio_dataset(data_dir, output_path, subset=['Bird', 'Fish', 'Butterfly']):
    ...
    for category in subset:
        ...
        actual_images = set(f for f in os.listdir(images_dir) 
                          if f.lower().endswith(('.jpg', '.jpeg', '.png'))) # Being flex.  I saw .JPG and .jpg in the dataset.
        ...
        logging.info(f"Missing images (in metadata but not in directory): {len(missing_images)}")
        logging.info(f"Unlisted images (in directory but not in metadata): {len(unlisted_images)}")
